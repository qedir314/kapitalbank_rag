...
flowchart LR
    ...
    D --> E[multilingual-e5-base\npassage embeddings]
    E --> F[(Chroma\n~2550 vectors)]
    C --> B2[BM25 lexical index\nbuilt from the same chunks]
    Q[User question] --> H[e5 query embedding]
    F --> R1[dense top-40]
    B2 --> R2[lexical top-40]
    H --> R1
    Q --> R2
...
| Embed | `ingest/embeddings.py` | Local `intfloat/multilingual-e5-base`, correct `query:`/`passage:` prefixes, L2-normalized |
...
- **Why local embeddings instead of an embeddings API?** Zero cost and zero data egress for the
  ingestion path — the right default for banking documents. `multilingual-e5` covers Azerbaijani,
...
|---|---|---|
| Retrieval hit@6 | 52% | share of questions where an expected source page made top-6 |
| MRR@6 | 0.46 | how high the first relevant page ranks |
| Judge faithfulness (1–5) | **4.84** | answers rarely claim anything beyond the context |
| Judge correctness (1–5) | **4.52** | questions get answered correctly despite retrieval noise |
...
The interesting gap — strong faithfulness/correctness against modest hit-rate — is the
classic RAG pattern: the generator compensates well for noisy top-k, and *retrieval*
(low-resource Azerbaijani embeddings + heavily consolidated product pages) is the
bottleneck to attack next with hybrid BM25 search and a cross-encoder re-ranker.
...
# 2) build the vector index (first run downloads the e5 model, ~1 GB)
